...
export class OrdemServicoComponent implements OnInit {
  ...
  openDialog(item?: WorkOrderDto) {
    if (item) {
      console.log("🚀 ~ file: ordem-servico.component.ts:249 ~ OrdemServicoComponent ~ openDialog ~ item:", item);
      const teste = item.responsibleMechanics.map((mecanico) => mecanico.name);
      console.log("🚀 ~ file: ordem-servico.component.ts:251 ~ OrdemServicoComponent ~ openDialog ~ teste:", teste);
      ...
      this.ordemServicoForm.patchValue({
        ...
        responsibleMechanics: item.responsibleMechanics.map((mecanico) => {
          return this.mecanicos.find(m => m.id === mecanico.id);
        }),
        closing: {
